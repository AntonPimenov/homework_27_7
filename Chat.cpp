...
void Chat::registerUser()
{
	...
	logger->log("Зарегистрирован новый пользователь: " + login);
}
// Вход в чат
bool Chat::login()
{
	...
	if (it != users.end() && it->second.password == password) {
		...
		cout << "Вход выполнен! Добро пожаловать, " << currentUser->name << "!\n";
		logger->log("Пользователь " + login + " вошел в чат!");	
		return true;
	}
...
void Chat::sendMessage()
{
	...
	logger->log("Пользователь " + currentUser->login + " отправил сообщение пользователю " + to + ": " + text);
}
...
void Chat::broadcastMessage()
{
	...
	// Логгируем отправку сообщения всем
	logger->log("Пользователь " + currentUser->login + " отправил сообщение всем: " + text);
}
//Просмотр сообщений
